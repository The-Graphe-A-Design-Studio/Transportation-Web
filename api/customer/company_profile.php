<?php
    ...
    if(isset($_POST['cu_id']) && isset($_POST['cu_name']) && isset($_POST['cu_email']) && isset($_POST['cu_phone_code']) && isset($_POST['cu_phone']) && 
        isset($_POST['cu_city']) && isset($_POST['cu_address']) && isset($_POST['cu_pin']) && isset($_POST['cu_pan']) && isset($_POST['cu_co_name']) && 
        isset($_POST['cu_co_type']) && isset($_POST['cu_co_service_tax']) && isset($_POST['cu_co_pan_num']) && isset($_POST['cu_co_website']))
    {
        $id = $_POST['cu_id'];
        $name = $_POST['cu_name'];
        ...
        $co_name = $_POST['cu_co_name'];
        $co_type = $_POST['cu_co_type'];
        $co_service_tax = $_POST['cu_co_service_tax'];
        $co_pan_num = $_POST['cu_co_pan_num'];
        $co_website = $_POST['cu_co_website'];
        ...
        {
            $sqle = "SELECT * FROM customers where cu_email = '$email' and cu_id <> '$id' and cu_type = '2'";
            ...
            else
            {
                ...
                $sqlr = "SELECT * FROM customers where cu_pan_num = '$pan' and cu_id <> '$id' and cu_type = '2'";
                ...
                {
                    $sqle = "SELECT * FROM customers where cu_co_service_tax = '$co_service_tax' and cu_id <> '$id' and cu_type = '2'";
                    $checke = mysqli_query($link, $sqle);
                    $rowe = mysqli_fetch_array($checke, MYSQLI_ASSOC);
                    $counte = mysqli_num_rows($checke);
                    if($counte >= 1)
                    ...
                    else
                    {
                        ...
                        $sqlr = "SELECT * FROM customers where cu_co_pan_num = '$co_pan_num' and cu_id <> '$id' and cu_type = '2'";
                        $checkr = mysqli_query($link, $sqlr);
                        $rowr = mysqli_fetch_array($checkr, MYSQLI_ASSOC);
                        $count = mysqli_num_rows($checkr);
                        if($count >= 1)
                        ...
                        else
                        {
                            ...
                            if($_POST['cu_phone_code'] == $row_c['cu_phone_code'] && $_POST['cu_phone'] == $row_c['cu_phone'])
                            {
                                $up_c = "update customers set cu_name = '".$_POST['cu_name']."', cu_email = '".$_POST['cu_email']."', 
                                        ...
                                        cu_co_name = '$co_name', cu_co_type = '$co_type', cu_co_service_tax = '$co_service_tax', cu_co_pan_num = '$co_pan_num', 
                                        cu_co_website = '$co_website' where cu_id = '".$_POST['cu_id']."'";
                            ...
                            else
                            {
                                ...
                                $mobile_sql = "insert into temp_register1 (cu_name, cu_email, cu_phone_code, cu_phone, cu_city, cu_address, 
                                                cu_pin_code, cu_pan_num, cu_co_name, cu_co_type, cu_co_service_tax, cu_co_pan_num, cu_co_website, t_otp, 
                                                t_reg_user) values ('$name', '$email', '$phone_code', '$phone', '$city', '$address', '$pin', '$pan', 
                                                '$co_name', '$co_type', '$co_service_tax', '$co_pan_num', '$co_website', '$rancu_no', '1')";
